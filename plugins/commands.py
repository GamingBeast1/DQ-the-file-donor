...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('Dɪssᴄᴜsɪᴏɴ ɢʀᴏᴜᴘ', url=GRP_LNK)
                ],[
                    InlineKeyboardButton('Mʏ ᴄᴏᴍᴀɴᴅs', callback_data='help'),
                    InlineKeyboardButton('Aʙᴏᴜᴛ ᴍᴇ', callback_data='about'),
                    InlineKeyboardButton('Iɴʟɪɴᴇ Sᴇᴀʀᴄʜ ☌', switch_inline_query_current_chat='')
                ],[
                    InlineKeyboardButton('Bᴀᴄᴋᴜᴘ Cʜᴀɴɴᴇʟ', url=CHNL_LNK)
                  ],[ InlineKeyboardButton('Oᴛᴛ ғʀᴇᴇ Aᴄᴄᴏᴜɴᴛs', url=https://t.me/get_OTT_Premium_Accounts ] ]
...
reply_markup = InlineKeyboardMarkup(buttons)
        await message.reply_photo(
            photo=random.choice(PICS),        reply_markup = InlineKeyboardMarkup(buttons)
        await message.reply_photo(
            photo=random.choice(PICS),
            caption=script.START_TXT.format(message.from_user.mention, temp.U_NAME, temp.B_NAME),
            reply_markup=reply_markup,
